// Thank you Jeremy P Bentham for the main program structure and the DMA library
// ADS7884 High speed ADC acquisition
#include <stdint.h>
#include <stdlib.h>
#include <unistd.h>
#include <stdio.h>
...
#define ADC_Res 8
#define ADC_Max ((uint16_t)(1)<<ADC_Res)*1.0
#define ADC_REQUEST(c)  {0xc0 | (c)<<5, 0x00}
#define ADC_VOLTAGE(n)  (((n) * 3.3) / ADC_Max)
#define ADC_MILLIVOLTS(n) ((int)((((n) * 3300) + 1024) / 2048))
// Modified ADC RAW VAL
#define ADC_RAW_VAL(d)  ((((uint16_t)(d)<<8 | (uint16_t)(d)>>8)>>(14-ADC_Res)) & 0x0ff)
...
typedef struct {
    DMA_CB cbs[NUM_CBS];
    uint32_t samp_size, pwm_val, adc_csd,adc_csd1, txd[2];
...
void adc_dma_init(MEM_MAP *mp, int nsamp, int single)
{
    ...
    ADC_DMA_DATA dma_data = {
        ...
        .adc_csd1= SPI_TFR_ACT | SPI_AUTO_CS | SPI_DMA_EN | SPI_FIFO_CLR | DAC_CE_NUM,
        ...
        // PWM ADC trigger: wait for PWM, set sample length, trigger SPI
            ...
            {PWM_TI,    MEM(mp, &dp->adc_csd),     REG(spi_regs, SPI_CS),   4, 0, CBS(10), 0}, // 9

            {PWM_TI,    MEM(mp, &dp->pwm_val),     REG(pwm_regs, PWM_FIF1), 4, 0, CBS(11), 0}, // 10
            {PWM_TI,    MEM(mp, &dp->samp_size),   REG(spi_regs, SPI_DLEN), 4, 0, CBS(12), 0}, // 11
            {PWM_TI,    MEM(mp, &dp->adc_csd1),     REG(spi_regs, SPI_CS),   4, 0, CBS(7), 0}, // 12
        }
    };
